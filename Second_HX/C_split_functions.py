...
def decision_variable_placement(x, enumerated_equipment, equipment_length):
    ...
    for index, equip in enumerated_equipment:
        ...
        elif equip == 6 and hx2_token == 1:
            approach_temp_2nd = x[index]
            hx2_token = 0
        elif equip == 6 and hx2_token == 0:
            x[index] = approach_temp_2nd
        elif equip == 9:
            split_ratio = x[index]
        ...
        # Main branch split ratio will be applied first if the 9 is always first equipment. Then the second branch split ratio will be applied later.
        elif equip == 10:
            # Rolling and unrolling to apply the split ratio to the second branch mass flow
            split_ratio_2nd = x[index]
            equipment = np.asarray(enumerated_equipment)[:, 1]
            rolled_eq = np.roll(equipment, -index, axis=0)
            mass_flow = np.roll(mass_flow, -index, axis=0)
            branching_start2 = 0
            branching_end3, branching_end4 = np.where(8 == rolled_eq)[0]
            for i in range(branching_start2, branching_end3):
                mass_flow[i] = mass_flow[i] * split_ratio_2nd
            for i in range(branching_end3, branching_end4):
                mass_flow[i] = mass_flow[i] * (1 - split_ratio_2nd)
            mass_flow = np.roll(mass_flow, index, axis=0)
...
def Pressure_calculation(
    ...
    if splitter2 == True:
        pos_splitter2 = np.where(10 == equipment)[0]
        mixer3, mixer4 = np.where(8 == equipment)[0]
        if pos_splitter2 > mixer3 and pos_splitter2 < mixer4:
            mixer3, mixer4 = mixer4, mixer3
    while Pressures.prod() == 0:
        ...
        if (
            splitter2 == True
            and Pressures[mixer3 - 1] != 0
            and Pressures[mixer4 - 1] != 0
        ):
            Pressures[mixer4] = min(Pressures[mixer3 - 1], Pressures[mixer4 - 1])
        if Pressures.prod() == 0:
            for i in range(len(Pressures)):
                if Pressures[i] != 0:
                    if i == len(Pressures) - 1:
                        ...
                        if equipment[0] == 6:
                            Pressures[0] = (
                                Pressures[i] - hx_pdrop
                            )  # Same hx_pdrop for both HXs
                        if equipment[0] == 9:
...
def splitter_mixer_calc_2(
    Temperatures, Pressures, enthalpies, entropies, mass_flow, equipment
):

    equipment = np.asarray(equipment)
    splitter = np.where(equipment == 10)[0]
    Temperatures[splitter] = Temperatures[splitter - 1]
    enthalpies[splitter] = enthalpies[splitter - 1]
    entropies[splitter] = entropies[splitter - 1]
    mixer1, mixer2 = np.where(equipment == 8)[0]
    if splitter > mixer1 and splitter < mixer2:
        mixer1, mixer2 = mixer2, mixer1
    if Temperatures[splitter] != 0 and Temperatures[mixer1] == 0:
        Temperatures[mixer1] = Temperatures[splitter]
        enthalpies[mixer1] = enthalpies[splitter]
        entropies[mixer1] = entropies[splitter]
...
def exergoeconomic_calculation(
    ...
    for i, j in enumerated_equipment:
        ...
        if j == 1:
            ...
            hx_token = 0
        elif j == 6 and hx2_token == 1:
            m1[i][hotside_index2 - 1] = -1 * exergies[hotside_index2 - 1]
            m1[i][hotside_index2] = exergies[hotside_index2]
            m1[i][coldside_index2 - 1] = -1 * exergies[coldside_index2 - 1]
            m1[i][coldside_index2] = exergies[coldside_index2]
            hxer_aux = np.copy(zero_row)
            hxer_aux[0][hotside_index2 - 1] = 1
            hxer_aux[0][hotside_index2] = -1
            m1 = np.concatenate((m1, hxer_aux), axis=0)
            hx2_token = 0
        elif j == 7:
