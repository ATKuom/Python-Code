...
if __name__ == "__main__":
    datalist = np.load(
        config.DATA_DIRECTORY / "v27msD10_tp9_m1_candidates.npy",
        allow_pickle=True,
    )
    # index = np.load(
    ...
    for i in range(len(datalist)):
        ...
        if i % 500 == 0 or i == len(datalist) - 1:
            print(len(valid_layouts), len(penalty_layouts), len(broken_layouts))
            positions_name = "v27mstp9D0_m2_positions" + str(i) + ".npy"
            results_name = "v27mstp9D0_m2_results" + str(i) + ".npy"
            np.save(
                config.DATA_DIRECTORY / positions_name,
                positions,
            )
