...
from ZW_model import GPT, PSI_LSTM
from ZW_Opt import *
...
psi_model = PSI_LSTM(64, 256)
psi_model.load_state_dict(torch.load("PSI_Models/psi_norm_aug_64_256_4_144_8.pt"))
model.eval()
psi_model.eval()
classes = std_classes
    ...
    def search(self, state):
        ...
        for search in range(self.args["num_searches"]):
            ...
            if not is_terminal:
                ...
                policy /= np.sum(policy)
                # psi evaluation of the state

                input = (self.game.get_encoded_state(node.state)).reshape(1, -1)
                lengths = torch.tensor([input.shape[1]]).long()
                value = 1 - psi_model(torch.tensor(input), lengths).item()
                node.expand(policy)
...
np.save(
    f"MCTS_Inference/MCTS_generated_designs_psi_{args['C']}_{args['num_searches']}_{args['dirichlet_epsilon']}_{args['dirichlet_alpha']}.npy",
    generated_designs,
)
